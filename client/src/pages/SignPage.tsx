        ...
        <>
            ...
            <main className="w-full font-inter min-h-screen bg-[url('/images/login_bg.png')] bg-cover bg-center text-white flex justify-center items-center   ">
                <div className={"flex  w-1/4 min-h-[500px] min-w-[250px] bg-black bg-opacity-80 items-center justify-center"}>
                    ...
                    <form onSubmit={(e:React.FormEvent<HTMLFormElement>)=>handleSubmit(e)} className={'flex w-full flex-col justify-center px-8 text-black  overflow-hidden py-4 max-[1090px]:px-4   '}>
                        <h2 className={"text-white text-3xl font-medium"}>Log In</h2>
                        <div className={"relative my-4 "}>
                            <input  type={"text"}  id="email" className={"pl-4 text-xl w-1/2  w-full px-1 py-1 h-[60px] text-base  outline-none group rounded-md peer max-[1090px]:h-[50px] "} value={email} onChange ={(e)=>setEmail(e.target.value)} required={true}/>
